...
2. Copy `.env.example` to `.env` and update credentials (`API_BASE_URL`, `SERVICE_TOKEN`, `AGENT_API_URL`). Optionally set `AGENT_HEALTH_URL` if the terrarium-agent health probe lives somewhere other than `<AGENT_API_URL before /v1>/health`, tune `STATUS_POLL_INTERVAL_SECONDS` / `LLM_STATUS_POLL_INTERVAL_SECONDS` as needed, and point `WORKER_WS_URL` (defaults to `<API_BASE_URL> -> ws://…/api/worker/updates`) at the relay’s new worker WebSocket.
   - If you want to stop polling when the worker WebSocket is connected, set `POLL_WHILE_WS_CONNECTED=false` (the worker will reconnect and resume polling automatically on disconnect).
   - `CHAT_LOG_DIR` controls where JSONL chat/tool logs are written (default `chat-logs/`). `ENHANCEMENT_DIR` controls where Terra’s enhancement requests are saved.
   - Set `LOG_ASSISTANT_CHUNKS=true` if you want every streamed token chunk logged; leave it `false` to log only final messages/tool events.
3. `poetry install`
